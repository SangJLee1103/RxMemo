        ...
        return CocoaAction { _ in
            return self.storage.createMemo(content: "")
                .flatMap { [unowned self] memo -> Observable<Void> in
                    let composeViewModel = MemoComposeViewModel(title: "세 메모", sceneCoordinator: self.sceneCoordinator, storage: self.storage, saveAction: self.performUpdate(memo: memo), cancelAction: self.performCancel(memo: memo))
